...
{"theme":"テーマ1行","summary":"サマリー300字","projectOverview":"プロジェクト全体の目的・背景・期待効果を300字程度で記載","promotionPoints":"推進にあたっての重要ポイント・注意点・成功の鍵を300字程度で記載","phases":[{"name":"名","period":"1〜3ヶ月目","goal":"ゴール1文","kpi":"KPI","actions":[{"title":"名","description":"1文","owner":"担当"}],"categoryActivities":{"初期設定":["具体的な活動"],"マニュアル作成":["具体的な活動"],"マニュアル活用":["具体的な活動"],"効果測定":["具体的な活動"],"その他":["具体的な活動"]}}]}`
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const callA = client.messages.create({
        ...
        messages: [{ role: 'user', content: `${context}\n\n4フェーズ生成。categoryActivitiesは全5カテゴリ×全4フェーズすべてに具体的な活動を1件以上必ず記入すること。活動内容は「現場担当者が無理なく取り組める」現実的でマイルドなトーンにすること。初期フェーズほど小さな成功体験を重視し、負担感を感じさせない表現・粒度にすること。projectOverviewとpromotionPointsもそれぞれ300字程度で必ず記入すること。\n${SCHEMA_PHASES}` }],
      }).then((r) => {
        try {
